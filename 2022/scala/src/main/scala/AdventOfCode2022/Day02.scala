...
object Day02 {
  def partOne(input: Seq[String]): Int = {
    val scores = Seq("B X", "C Y", "A Z", "A X", "B Y", "C Z", "C X", "A Y", "B Z").zipWithIndex.toMap.transform((_, v) => v + 1)
    input.map(scores).sum
  }

  def partTwo(input: Seq[String]): Int = {
    val scores = Seq("B X", "C X", "A X", "A Y", "B Y", "C Y", "C Z", "A Z", "B Z").zipWithIndex.toMap.transform((_, v) => v + 1)
    input.map(scores).sum
  }
